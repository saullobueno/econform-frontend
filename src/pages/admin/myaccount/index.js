...
import { useToast, Button, ButtonGroup, VStack, VisuallyHiddenInput, Flex, SimpleGrid } from '@chakra-ui/react';
...
export default () => {
  ...
  const handleConstruct = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await AdminUser.readItem(auth.id);
      reset({ ...data, password: '' });
    } catch (error) {
      toast({
  ...
  const handleOnSubmit = useCallback(
    async (data) => {
      ...
      setLoading(true);
      try {
        await AdminUser.updateItem(auth.id, data);
        toast({
  ...
  return (
    <>
      <Breadcrumbs
        title="Minha conta"
        pages={[{ page: localStorage.getItem('@eConform-ClientName') || 'Início', link: '/admin/dashboard' }]}
      />
      <Content>
        <Paper title="Meu perfil" loading={loading} isSubmitting={isSubmitting} fluid>
        ...
        <Paper title="Alterar senha" loading={loading} isSubmitting={isSubmitting} fluid>
          <VStack as="form" id="formRef" onSubmit={handleSubmit(handleOnSubmit)} spacing="8" align="stretch">
            <SimpleGrid minChildWidth={{ base: '320px', '6xl': '380px' }} spacing="8">
